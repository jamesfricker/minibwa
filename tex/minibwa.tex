...
BWA-MEM is the primary bottleneck for both performance and cost in variant calling pipelines.
...
It is typically 50--100\% faster while producing identical alignment.
BWA-MEME~\citep{Jung:2022aa} further speeds up BWA-MEM2 using learned indices while still producing identical output.
...
However, designed more than a decade ago, BWA-MEM has several limitations that prevent further optimization; these can be addressed if we accept breaking changes.
...
Minibwa falls back to SIMD-based dynamic programming~\citep{Suzuki:2018aa} when an ungapped alignment contains many mismatches.
...
minibwa prefilters the read-reference pair by testing $q$-mer matches.
More exactly,
$$
...
Minibwa only applies Smith-Waterman if $\max_t M_t\ge10$,
...
It then finds seeds against the converted FM-index.
...
Minibwa applies chaining normally and performs base alignment and mate rescue entirely in the 3-nucleotide space.
A weakness of this approach is that a reference T base matching a read C base would not be penalized.
BISCUIT~\citep{Zhou:2024ac} addresses this issue by doing alignment in the normal 4-nucleotide space
with an asymmetric scoring matrix that penalizes a T-to-C mismatch but not C-to-T mismatch.
It uses a better algorithm in theory.
...
BWA-MEM3 v0.2.1, Bowtie2 v2.5.5~\citep{Langmead:2012fk}, minimap2 v2.30~\citep{Li:2018ab} and strobealign v0.17.0~\citep{Sahlin:2022aa} for short reads,
...
BWA-MEM is slightly more accurate than minibwa for WGS short reads (Fig.~\ref{fig:roc}a and~\ref{fig:roc}b).
The difference is primarily driven by centromeric or acrocentric regions (Fig.~\ref{fig:roc}b versus~\ref{fig:roc}c)
as minibwa extends fewer reads in such regions (Section~\ref{sec:align}).
...
BWA-Meth is not as accurate due to its limitations explained in Introduction.
Bismark is known to map fewer reads than BWA-Meth~\citep{Gong_2022}.
It is unclear why BISCUIT is less accurate than minibwa despite using a theoretically superior strategy (Section~\ref{sec:meth}).
...
For standard WGS short reads, strobealign is the fastest and Bowtie2 is the most memory efficient.
...
The gap between minibwa and other BWA-MEM implementations is smaller for Hi-C and Roche's single-end SBX reads,
probably due to the lack of mate rescue where minibwa has an advantage with more heuristics (Section~\ref{sec:pe}).
For long reads, minibwa is a little faster than minimap2 and is an order of magnitude faster than Winnowmap2.

\subsection{Effect on short-read variant calling}

\begin{table}[b]
\caption{Short-read variant calling accuracy}\label{tab:var}
\tabcolsep=0pt%%
...
Aligner & FN & FP & FN & FP \\
\midrule
BWA-MEM & 46,895 & 7,585 & 37,425 & 5,218 \\
Minibwa & 46,367 & 7,544 & 36,321 & 5,308 \\
\botrule
\end{tabular*}
\end{table}
...
As is shown in Table~\ref{tab:var}, minibwa closely matches BWA-MEM on small variant calling accuracy.
...
enabling long-read alignment and doubling performance over BWA-MEM2.
