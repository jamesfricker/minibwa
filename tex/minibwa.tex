	...
	\begin{algorithmic}[1]
		\Procedure{FindSMEM}{$\mathcal{P},\ell,c$}
			\ForAll{$P\in\mathcal{P}$}
				\State $Q.{\rm push}(1,P,0,\cdot,\cdot)$\Comment{$Q$ is a queue}
			\EndFor
			\While{$Q$ is not empty}
				\State $(u,P,i,j,(k,k',s))\gets Q.{\rm shift}()$\Comment{take out the head}
				\If{$u=1$}\Comment{stage 1: prepare}
					\State $Q.{\rm push}(2,P,i,i+\ell-1,(0,0,|B|))$
				\ElsIf{$u=2$}\Comment{stage 2: backward extension}
					\If{$j<i$}\Comment{move to stage 3}
						\State $Q.{\rm push}(3,P,i,i+\ell,(k,k',s))$
					\ElsIf{$s\ge c$}\Comment{1-step backward ext.}
						\State $Q.{\rm push}(2,P,i,j-1,${\sc BackExt}$((k,k',s),P[j]))$
					\Else\Comment{no SMEM possible; back to stage 1}
						\State $Q.{\rm push}(1,P,j+1,\cdot,\cdot)$
					\EndIf
				\ElsIf{$u=3$}\Comment{stage 3: forward extension}
					...
					\If{$t\ge c$}\Comment{1-step forward ext.}
						\State $Q.{\rm push}(3,P,i,j+1,(l,l',t))$
					\Else\Comment{move to stage 4}
						\State Output $(P,i,j,(k,k',s))$\Comment{report an SMEM}
						\State $Q.{\rm push}(4,P,i,j-1,(0,0,B))$
					\EndIf
				\ElsIf{$u=4$}\Comment{stage 4: backward ext. again}
					\If{$s\ge c$}\Comment{1-step backward ext.}
						\State $Q.{\rm push}(4,P,i,j-1,${\sc BackExt}$((k,k',s),P[j]))$
					\Else\Comment{back to stage 1}
						\State $Q.{\rm push}(1,P,j+1,\cdot,\cdot)$
					\EndIf
				\EndIf
			\EndWhile
